...
export function Dashboard() {
  ...
  const totalExpenses = expenses.reduce((sum, expense) => sum + expense.amount_in_default_currency, 0);
  const thisMonthExpenses = expenses.filter(expense => {
    const expenseDate = new Date(expense.expense_date);
    const now = new Date();
    return expenseDate.getMonth() === now.getMonth() && expenseDate.getFullYear() === now.getFullYear();
  });
  const thisMonthTotal = thisMonthExpenses.reduce((sum, expense) => sum + expense.amount_in_default_currency, 0);
  
  const categories = expenses.reduce((acc, expense) => {
    const categoryName = expense.category?.name || 'Uncategorized';
    acc[categoryName] = (acc[categoryName] || 0) + expense.amount_in_default_currency;
    return acc;
                ...
                {recentExpenses.map((expense, index) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <span className={`inline-block px-2 py-1 text-xs rounded-full mt-1 ${getStatusBadge(expense.status)}`}>
                    ...
                    <motion.div 
                      ...
                      whileHover={{ scale: 1.05 }}
                    >
                      -${expense.amount_in_default_currency.toFixed(2)}
                    </motion.div>
                  </motion.div>
                ))}
              </div>
